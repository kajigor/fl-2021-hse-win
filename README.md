...
<img src="images/4f04b1593a4de66b94808d7f4e94042cbb5c0cfe6f042b409319557ee3b1cade.png" alt="drawing" style="width:800px;"/>

<h5>Задание 2:</h5>

<img src="images/1019cbaf1c315851ee13f750f1c06895b23032483b23b568e4c2b88c8507a0ef.png" alt="drawing" style="width:800px;"/>
...
<br/>
Давайте позволим составителям конечных автоматов "руками" создавать состояния (вершины) и переходы (рёбра).
<br/>
В нашем языке сначала требуется описать алфавит - это "enum", потом все состояния - тоже "enum", далее все рёбра - тоже "enum", все вершины, рёбра, символы из алфавита обязаны иметь уникальный индификатор!
Далее задаётся начальная верщина - элемент из множества вершин

Далее должно идти ключевое слово class, потом есть 2 варианта: будет описание ребра или состояния.
Состояние описывается 4 параметрами:
id вершины, массив из исходящих рёбер, комментарий - строковый литерал, далее bool значение - является ли вершина терминальной.

Ребро описывается 5 параметрами:
id ребра, массив из символов алфавита, которые показывают, каким символам они разрешают идти, состояния начала и конца, комментарий - строковый литерал

у всех параметров должен быть одинаковый отступ от начала строки, описание параметров имеет вид:
parameter : value

пробелы показывают вложенность объектов друг в друга
