...
def t_TERMINAL_VERTEX(t):
    ...
    tmp = t.value
    ends.append(tmp[1:-3])
    vertices[tmp[:-3]] = len(vertices)


def t_COMMON_VERTEX(t):
    r"""q[0-9]+\(\)"""
    tmp = t.value
    vertices[tmp[:-2]] = len(vertices)
...
if cnt_checks != cnt_passed:
    cpp_file.write("checks failed")
    sys.exit(0)
else:
    cpp_file.write("checks passed\n")
cpp_file.write(f"{len(alphabet)} ")
for c in alphabet.keys():
    cpp_file.write(f"{c} ")
cpp_file.write(f"\n{len(vertices)} ")
cpp_file.write(f"{len(ends)} ")
cpp_file.write(f"{len(alphabet) * len(vertices)}\n")
cpp_file.write(f"{vertices[start_vertex]}\n")
for q in ends:
    cpp_file.write(f"{q} ")
cpp_file.write('\n')
for e in edges.keys():
    for c in edges[e]:
        cpp_file.write(f"{vertices[e[0]]} {vertices[e[1]]} {alphabet[c]}\n")
