...
def t_return(t):
    ...
    file_out.write(stack[-1] + to_arrow + new_vertex + "\n")
    file_out.write(new_vertex + to_arrow + create_value_vertex(get_return_value(t.value)) + "\n")
    return t
...
def t_condition(t):
    r"""[^\;&^\)]+(\;|\))"""
    file_out.write(stack[-1] + to_arrow + create_name("Condition=\"" + get_condition(t.value) + "\"") + "\n")
    return t
